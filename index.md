...
> **Related work sample:** [AI-Agent Security Assessment — Damn Vulnerable LLM Agent](/dvla/DVLA-Security-Assessment-Report.html) — a full penetration-test report against a deliberately-vulnerable LLM banking agent: prompt injection → SQL injection → credential dump, broken access control (IDOR), a concealed breach, and system-prompt leakage, each reproduced and mapped to the OWASP LLM Top 10. [[PDF]](/dvla/DVLA-Security-Assessment-Report.pdf)
...
This is now a whole class. Composio, Pipedream, Smithery's and Glama's hosting, and smaller players all market the same model, often as a feature: "centralized credential management," keys "encrypted at rest," "never exposed to the model." One concrete instance — `pipeworx-io` on GitHub — hosts on the order of 1,148 public connector repositories, almost all thin, zero-star, templated wrappers that route through a single gateway endpoint. That repo count is verifiable on GitHub (as of the 2026-06-29 snapshot); its self-reported tool and source counts are marketing figures I couldn't independently verify, so I don't cite them. Its own README is explicit: credential handling happens server-side. You don't manage keys, the gateway does.
